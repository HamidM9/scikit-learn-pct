...
cdef class ClusEntropy(ClassificationCriterion):
    ...
    cdef float64_t* _tw
    cdef cnp.uint8_t* _y_missing
    cdef bint _has_tw

    def __cinit__(self):
        self._tw = NULL
        self._has_tw = False
        self._y_missing = NULL
    ...
    def __dealloc__(self):
        ...
        if self._y_missing != NULL:
            free(self._y_missing)
            self._y_missing = NULL
...
cdef class ClusModifiedEntropy(ClassificationCriterion):
    """CLUS modified entropy: Laplace-smoothed, summed over outputs, base-2."""
    cdef float64_t * _tw
    cdef bint _has_tw
    cdef cnp.uint8_t * _y_missing
    cdef Py_ssize_t _n_samples
    cdef Py_ssize_t _n_outputs
    ...
    def set_y_missing_mask(self, cnp.ndarray[cnp.uint8_t, ndim=2] mask):
        cdef Py_ssize_t i
        if mask.shape[1] != self.n_outputs:
            raise ValueError("mask shape mismatch")
        if self._y_missing != NULL:
            free(self._y_missing)
            self._y_missing = NULL

        self._n_samples = mask.shape[0]
        self._n_outputs = mask.shape[1]
        self._y_missing = <cnp.uint8_t *> malloc(self._n_samples * self._n_outputs * sizeof(cnp.uint8_t))
        ...
        for i in range(self._n_samples * self._n_outputs):
            self._y_missing[i] = mask.ravel()[i]

    cdef inline float64_t _w(self, intp_t o) noexcept nogil:
        return self._tw[o] if self._has_tw else 1.0

    cdef float64_t node_impurity(self) noexcept nogil:
        cdef float64_t impur = 0.0
        cdef float64_t w, prob
        cdef float64_t total, denom
        cdef intp_t o, c
        ...
        for o in range(self.n_outputs):
            ...
            total = self.weighted_n_node_samples
            if total <= 1e-12:
                continue

            denom = total + self.n_classes[o]

            for c in range(self.n_classes[o]):
                if self.sum_total[o][c] > 0.0:
                    prob = (self.sum_total[o][c] + 1.0) / denom
                    impur -= w * prob * log(prob) * invln2
    ...
    cdef void children_impurity(self, float64_t* impurity_left,
        ...
        for o in range(self.n_outputs):
            ...
            if self.weighted_n_left > 1e-12:
                denom = self.weighted_n_left + self.n_classes[o]
                for c in range(self.n_classes[o]):
                    if self.sum_left[o][c] > 0.0:
                        prob = (self.sum_left[o][c] + 1.0) / denom
                        left -= w * prob * log(prob) * invln2

            if self.weighted_n_right > 1e-12:
                denom = self.weighted_n_right + self.n_classes[o]
                for c in range(self.n_classes[o]):
                    if self.sum_right[o][c] > 0.0:
                        prob = (self.sum_right[o][c] + 1.0) / denom
                        right -= w * prob * log(prob) * invln2
